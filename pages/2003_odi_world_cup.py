...
dash.register_page(__name__, name = '2003 ODI WORLD CUP', order=1111111111100000)

df = pd.read_excel('assets/2003_World_Cup/points_table_2003.xlsx')
df = df.fillna(' ')
df = df[['SUPER SIXES','TEAMS','MATCHES','W','L','T','N/R','POINTS','NRR']]
df1 = df[:6]
...
fig2 = go.Figure(data=[go.Pie(labels=df2.TEAMS, values=df2.POINTS, pull=[0, 0, 0, 0, 0, 0, 0.2])])
fig3 = go.Figure(data=[go.Pie(labels=df3.TEAMS, values=df2.POINTS, pull=[0, 0, 0, 0, 0, 0, 0.2])])
...
def layout():
    ...
    dbc.Row(
        [
            ...
            dbc.Col(
                [
                    html.H3('POINTS TABLE - 2003 ODI WORLD CUP', style={'textAlign': 'center'}),
                    html.Br(),
