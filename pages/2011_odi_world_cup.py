...
dash.register_page(__name__, name = '2011 ODI WORLD CUP', order=1111111111111000)

df = pd.read_excel('assets/2011_World_Cup/points_table_2011.xlsx')
df = df.fillna(' ')
df = df[['GROUP A','TEAMS','MATCHES','W','L','T','N/R','POINTS','NRR']]
df1 = df[:7]
...
fig1 = go.Figure(data=[go.Pie(labels=df1.TEAMS, values=df1.POINTS, pull=[0, 0, 0, 0, 0, 0, 0.2])])
...
def layout():
    ...
    dbc.Row(
        [
            ...
            dbc.Col(
                [
                    html.H3('POINTS TABLE - 2011 ODI WORLD CUP', style={'textAlign': 'center'}),
                    html.Br(),
