...
def initialize_data(year):
    ...
    batsman_df = df[df['ROLE'].isin(bat_options)]
    batsman_df = batsman_df[batsman_df['YEAR'] >= year]
    # batsman_and_wk_df = batsman_and_wk_df[batsman_and_wk_df['NO_OF_MATCHES_PLAYED']>=1]
    # batsman_df.head()

    batsman_df = batsman_df[
        ['NO_OF_MATCHES_PLAYED', 'RUNS', 'BATTING_AVG', 'BATTING_STRIKE_RATE', 'RECENTLY_PLAYING_IN_ODI']]
    # batsman_df.shape

    batsman_df[['NO_OF_MATCHES_PLAYED', 'RUNS', 'BATTING_AVG', 'BATTING_STRIKE_RATE', 'RECENTLY_PLAYING_IN_ODI']] = \
    batsman_df[
        ['NO_OF_MATCHES_PLAYED', 'RUNS', 'BATTING_AVG', 'BATTING_STRIKE_RATE', 'RECENTLY_PLAYING_IN_ODI']].astype(float)
    # batsman_df.info()

    # print(batsman_df.isnull().sum())

    data = batsman_df[['NO_OF_MATCHES_PLAYED', 'RUNS', 'BATTING_AVG', 'BATTING_STRIKE_RATE', 'RECENTLY_PLAYING_IN_ODI']]
    ...
    x_train = df_train[['NO_OF_MATCHES_PLAYED', 'RUNS', 'BATTING_AVG', 'BATTING_STRIKE_RATE']]
    x_test = df_test[['NO_OF_MATCHES_PLAYED', 'RUNS', 'BATTING_AVG', 'BATTING_STRIKE_RATE']]
...
def model_predictions(team, year):
    ...
    batsman_and_wk_df = df[df['COUNTRY'] == team]
    ...
    batsman_and_wk_df = batsman_and_wk_df[batsman_and_wk_df['RECENTLY_PLAYING_IN_ODI'] == 'YES']
    bat_and_wk = batsman_and_wk_df[batsman_and_wk_df['RECENTLY_PLAYING_IN_ODI'] == 'YES']
    batsman_and_wk_df = batsman_and_wk_df[['PLAYERS','NO_OF_MATCHES_PLAYED', 'RUNS', 'BATTING_AVG', 'BATTING_STRIKE_RATE']].copy()
    #print(batsman_and_wk_df)
...
#prob = model_predictions('BANGLADESH',2021)
