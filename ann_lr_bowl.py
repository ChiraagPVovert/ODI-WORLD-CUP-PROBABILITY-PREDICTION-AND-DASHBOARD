...
def initialize_data(year):
    ...
    bat_options = ['Bowler']
    bowler_df = df[df['ROLE'].isin(bat_options)]
    bowler_df = bowler_df[bowler_df['YEAR'] >= year]
    # bowler_df = bowler_df[bowler_df['NO_OF_MATCHES_PLAYED']>=1]
    # bowler_df.head()

    bowler_df = bowler_df[
        ['NO_OF_MATCHES_PLAYED', 'WICKETS', 'BOWLING_AVERAGE', 'BOWLING_STRIKE_RATE', 'ECONOMY',
         'RECENTLY_PLAYING_IN_ODI']]
    # bowler_df.shape

    bowler_df[['NO_OF_MATCHES_PLAYED', 'WICKETS', 'BOWLING_AVERAGE', 'BOWLING_STRIKE_RATE', 'ECONOMY',
               'RECENTLY_PLAYING_IN_ODI']] = bowler_df[
        ['NO_OF_MATCHES_PLAYED', 'WICKETS', 'BOWLING_AVERAGE', 'BOWLING_STRIKE_RATE', 'ECONOMY',
         'RECENTLY_PLAYING_IN_ODI']].astype(float)
    # bowler_df.info()
    ...
    x_train = df_train[['NO_OF_MATCHES_PLAYED', 'WICKETS', 'BOWLING_AVERAGE', 'BOWLING_STRIKE_RATE', 'ECONOMY']]
    x_test = df_test[['NO_OF_MATCHES_PLAYED', 'WICKETS', 'BOWLING_AVERAGE', 'BOWLING_STRIKE_RATE', 'ECONOMY']]
...
def model_predictions(team, year):
    ...
    x_train, x_test, y_train, y_test = initialize_data(year)
    ...
    bowler_df = bowler_df[bowler_df['RECENTLY_PLAYING_IN_ODI'] == 'YES']
    bowler_df = bowler_df[
        ['PLAYERS', 'NO_OF_MATCHES_PLAYED', 'WICKETS', 'BOWLING_AVERAGE', 'BOWLING_STRIKE_RATE', 'ECONOMY']].copy()
    ...
    final_bat_predict_df = pd.concat([bowler_df, final_bowl_predict_df], axis=1, join='inner')
